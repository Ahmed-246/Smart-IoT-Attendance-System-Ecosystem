...
import { Shield, ShieldCheck, Search, Activity, Clock, AlertTriangle, ShieldAlert, ExternalLink, UserCog } from 'lucide-react';
...
export default function AdminsPage() {
  ...
  const [form, setForm] = useState({ name: '', phone_number: '', email: '', password: '', role: 'admin', academic_password: '' });
  const [search, setSearch] = useState('');
  const { toast, ToastContainer } = useToast();
  const { role: currentUserRole, isSuperAdmin } = useAuth();
  ...
  async function loadAudit() {
    ...
    try {
      ...
      // Filter logs to show only admin/super_admin related actions if possible, 
      // but for now we show all user logs related to management
  ...
  async function load() {
    ...
    try { 
      ...
      // Filter only Admins and Super Admine
      const filtered = r.data.filter(u => u.role === 'admin' || u.role === 'super_admin');
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      toast('Administrator created'); setModal(false); setForm({ name: '', phone_number: '', email: '', password: '', role: 'admin', academic_password: '' }); load();
  ...
  async function handleEdit(e) {
    ...
    try {
      ...
      toast('Account updated'); setEditModal(null); load();
    } catch (err) { toast(err.response?.data?.detail || 'Error', 'error'); }
  }
  ...
  async function handleDelete() {
    try {
      ...
      toast('Account deleted'); setDeleteConfirm(null); load();
    } catch (err) { toast(err.response?.data?.detail || 'Error', 'error'); }
  }
  ...
  const columns = [
    { key: 'name', label: 'Name', render: (v, row, i) => (
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        <span style={{ color: 'var(--text-muted)', fontSize: 11 }}>#{i+1}</span>
        <Link to={`/users/${row.id}`} style={{ fontWeight: 600, color: 'inherit', textDecoration: 'none' }} className="hover-link">
          {v || '—'}
        </Link>
      </div>
    )},
    ...
    { key: 'role', label: 'Role', render: v => {
      const config = roleConfig[v] || { color: 'default', icon: Shield, label: v };
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 20 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            <UserCog size={24} color="var(--orange)" />
            <h1 style={{ fontSize: 20, fontWeight: 600 }}>Administrative Team</h1>
          </div>
          <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>Manage system administrators and their security levels.</p>
        </div>
        ...
        {isSuperAdmin && (
          <>
            <Btn variant="ghost" onClick={loadAudit} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Activity size={16} /> Admin Activity
            </Btn>
            <Btn onClick={() => { setForm({ name: '', phone_number: '', email: '', password: '', role: 'admin', academic_password: '' }); setModal(true); }}>+ Add Admin</Btn>
          </>
        )}
      </div>
      
      <Card style={{ padding: 0 }}>
        <Table columns={columns} rows={filteredUsers} emptyText={search ? "No administrators match your search" : "No administrators found"} />
      </Card>

      {modal && (
        <Modal title="Create New Administrator" onClose={() => setModal(false)}>
          <form onSubmit={handleAdd} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <Field label="Full Name"><Input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Enter full name" /></Field>
            <Field label="Phone Number"><PhoneInput value={form.phone_number} onChange={e => setForm(f => ({ ...f, phone_number: e.target.value }))} /></Field>
            <Field label="Work Email"><Input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} required placeholder="email@university.edu" /></Field>
            <Field label="Security Password"><PasswordInput value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} required /></Field>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <Btn type="submit">Deploy Admin</Btn>
            </div>
          </form>
        </Modal>
      )}
      ...
      {editModal && (
        <Modal title="Edit Administrative Account" onClose={() => setEditModal(null)}>
          <form onSubmit={handleEdit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <Field label="Change Password (Optional)"><PasswordInput value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="Leave blank to keep unchanged" /></Field>
            {isSuperAdmin && (
              <Field label="Role">
                <Select value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}>
      ...
      {deleteConfirm && (
        <ConfirmModal 
          ...
          message={`Are you sure you want to permanently delete the administrative account for ${deleteConfirm.email}?`}
      ...
      {auditModal && (
        <Modal title="Administrative Audit Logs" onClose={() => setAuditModal(false)} width={800}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxHeight: 500, overflowY: 'auto', padding: '10px 4px' }}>
            {auditLoading ? (
              ...
              <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>No logs found.</div>
            ) : (
              auditLogs.map(log => (
                  ...
                  display: 'flex', gap: 16, alignItems: 'center'
                   ...
                   <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4, display: 'flex', gap: 12 }}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>By: {log.user_email}</span>
                        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>At: {new Date(log.timestamp).toLocaleString()}</span>
                      </div>
                   </div>
                </div>
              ))
