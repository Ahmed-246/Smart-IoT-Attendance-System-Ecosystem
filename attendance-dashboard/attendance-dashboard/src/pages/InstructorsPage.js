...
import { instructorsApi, facultiesApi, departmentsApi } from '../api/client';
...
const InstructorForm = ({ onSubmit, onCancel, submitLabel, form, setForm, faculties, departments }) => {
  const addAssignment = () => {
    setForm(prev => ({
      ...prev,
  ...
  return (
    <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: 12 }}>
        <Field label="Title">
          <Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Eng." />
        </Field>
        <Field label="Full name">
          <Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Ahmed Hassan" required />
        </Field>
      </div>

      <Field label="Email (used for login)">
        <Input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="ahmed@school.edu" required />
      </Field>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <Field label="Phone Number">
          <PhoneInput value={form.phone_number} onChange={e => setForm(f => ({ ...f, phone_number: e.target.value }))} />
        </Field>
        <Field label="Specialization"><Input value={form.specialization} onChange={e => setForm(f => ({ ...f, specialization: e.target.value }))} placeholder="Embedded Systems" /></Field>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <Field label="Office Hours"><Input value={form.office_hours} onChange={e => setForm(f => ({ ...f, office_hours: e.target.value }))} placeholder="Mon 10-12, Wed 2-4" /></Field>
        <Field label="Appointment Link"><Input value={form.appointment_link} onChange={e => setForm(f => ({ ...f, appointment_link: e.target.value }))} placeholder="calendly.com/instructor" /></Field>
      </div>

      <Field label="Instructor Bio"><Input value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} placeholder="Brief background..." /></Field>
...
export default function InstructorsPage() {
  ...
  const emptyForm = { 
    name: '', email: '', title: 'Eng.', 
    phone_number: '', specialization: '', office_hours: '', bio: '', appointment_link: '',
  ...
  async function load() {
    setLoading(true);
    try { 
      const [inst, fac, dep] = await Promise.all([
        instructorsApi.list(),
        facultiesApi.list(),
        departmentsApi.list()
      ]);
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      toast('Instructor added. Default password: Instructor@1234');
  ...
  async function handleEdit(e) {
    ...
    try {
      ...
      await instructorsApi.update(editModal.id, payload);
      toast('Instructor updated'); setEditModal(null); load();
    } catch (err) { toast(err.response?.data?.detail || 'Error', 'error'); }
  }
  ...
  async function handleDelete() {
    try {
      ...
      toast('Instructor deleted'); setDeleteConfirm(null); load();
    } catch (err) { toast(err.response?.data?.detail || 'Error', 'error'); }
  }

  function openEdit(instructor) {
    const existingAssignments = (instructor.faculties || []).map((fac, i) => ({
      faculty_id: fac.id,
      department_id: instructor.departments?.[i]?.id || ''
    })) || [{ faculty_id: '', department_id: '' }];

    setForm({ 
      name: instructor.name, 
      email: instructor.email, 
      title: instructor.title || 'Eng.',
      phone_number: instructor.phone_number || '',
      specialization: instructor.specialization || '',
      office_hours: instructor.office_hours || '',
      bio: instructor.bio || '',
      appointment_link: instructor.appointment_link || '',
      assignments: existingAssignments.length > 0 ? existingAssignments : [{ faculty_id: '', department_id: '' }]
    });
    setEditModal(instructor);
  }

  const columns = [
    { key: 'id', label: 'ID', render: (_, __, i) => <span className="mono" style={{ color: 'var(--text-muted)' }}>#{i + 1}</span> },
    { key: 'title', label: 'Title', render: v => <Badge color="blue">{v || 'Eng.'}</Badge> },
    { key: 'name', label: 'Name', render: (v, row) => (
      <span 
        style={{ fontWeight: 600, color: 'var(--accent)', cursor: 'pointer', textDecoration: 'underline' }}
        onClick={() => navigate(`/instructors/${row.id}`)}
      >
        {v}
      </span>
    )},
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 600, marginBottom: 4 }}>Instructors</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>{instructors.length} registered</p>
        </div>
        <Btn onClick={() => { setForm(emptyForm); setModal(true); }}>+ Add Instructor</Btn>
      </div>
      <div style={{
        ...
        Adding an instructor also creates their login account with password <code style={{ fontFamily: 'var(--font-mono)' }}>Instructor@1234</code>. Ask them to change it after first login.
      </div>
      <Card style={{ padding: 0 }}>
        <Table columns={columns} rows={instructors} maxHeight="480px" emptyText="No instructors yet" />
      </Card>

      {modal && (
        <Modal title="Add Instructor" onClose={() => setModal(false)}>
          <InstructorForm 
            form={form} setForm={setForm} faculties={faculties} departments={departments}
            onSubmit={handleAdd} onCancel={() => setModal(false)} submitLabel="Add Instructor" 
          />
        </Modal>
      )}

      {editModal && (
        <Modal title="Edit Instructor" onClose={() => setEditModal(null)}>
          <InstructorForm 
            form={form} setForm={setForm} faculties={faculties} departments={departments}
            onSubmit={handleEdit} onCancel={() => setEditModal(null)} submitLabel="Save Changes" 
          />
        </Modal>
      )}
      ...
      {deleteConfirm && (
        <ConfirmModal 
          ...
          message={`Are you sure you want to delete ${deleteConfirm.name}? This will also permanently remove their academic user account.`}
