...
export default function InstructorProfilePage() {
  ...
  async function load() {
    ...
    try {
      ...
      toast(err.response?.data?.detail || 'Failed to load instructor profile', 'error');
    } finally {
      setLoading(false);
    }
  }
  ...
  if (!data) return <div style={{ padding: 40, color: 'var(--text-muted)' }}>Instructor not found</div>;

  const { instructor, assigned_courses } = data;

  // Derive stats
  const totalCourses = assigned_courses.length;
  const totalDepartments = uniqueDepts.length;
  const activeFaculties = instructor.faculties?.length || 0;
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 28, paddingBottom: 60 }}>
      ...
      <div>
        <Btn variant="ghost" size="sm" onClick={() => navigate('/instructors')} icon={<ChevronLeft size={16} />}>
          Back to Engineers
        </Btn>
      </div>
        ...
        <div style={{ padding: '40px 48px', display: 'flex', gap: 40, alignItems: 'center', position: 'relative', zIndex: 1, flexWrap: 'wrap' }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{
              ...
              background: instructor.profile_image_url ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg, rgba(245,158,11,0.2) 0%, rgba(217,119,6,0.05) 100%)',
              padding: 4, 
              transform: instructor.profile_image_url ? 'none' : 'rotate(-4deg)', 
              transition: 'transform 0.4s ease',
              border: instructor.profile_image_url ? '1px solid rgba(245,158,11,0.2)' : 'none'
               }} className="hover-rotate">
               <div style={{
                 width: '100%', height: '100%', borderRadius: '22%',
                 background: instructor.profile_image_url ? 'transparent' : 'linear-gradient(135deg, #f59e0b, #d97706)',
                 display: 'flex', alignItems: 'center', justifyContent: 'center',
                 fontSize: 48, fontWeight: 800, color: '#000',
                 boxShadow: instructor.profile_image_url ? 'none' : 'inset 0 0 20px rgba(255,255,255,0.4)',
                 overflow: 'hidden',
                 position: 'relative'
               }}>
                 {instructor.profile_image_url ? (
                   <img 
                    src={instructor.profile_image_url.startsWith('http') ? instructor.profile_image_url : `${instructor.profile_image_url}`} 
                    alt={instructor.name}
            ...
            <Badge color="amber" style={{ 
              position: 'absolute', bottom: -10, left: '50%', transform: 'translateX(-50%)', 
              padding: '6px 16px', fontSize: 13, fontWeight: 800, 
              boxShadow: '0 8px 24px rgba(0,0,0,0.6)', 
              border: '1px solid rgba(245,158,11,0.5)', 
              background: '#92400e', // Solid deep amber for contrast
              color: '#fff',
              zIndex: 2
            }}>
              ENGINEER
          ...
          <div style={{ flex: 1 }}>
            <h1 style={{ fontSize: 36, fontWeight: 800, margin: '0 0 8px 0', color: '#fff', letterSpacing: '-0.02em' }}>
              {instructor.title && <span style={{ color: 'var(--amber)', marginRight: 12 }}>{instructor.title}</span>}
              {instructor.name}
            </h1>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 18, color: 'var(--amber)', fontWeight: 600, letterSpacing: '0.02em', marginBottom: 20 }}>
              <Award size={20} />
              {instructor.specialization || 'Engineering Faculty'}
            </div>
            ...
            <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
              ...
              {instructor.phone_number && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, color: 'var(--text-secondary)' }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)' }}>{formatPhoneNumber(instructor.phone_number)}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Action Button */}
           {instructor.appointment_link && (
            <div style={{ alignSelf: 'center', minWidth: 200 }}>
              <Btn 
                variant="accent" 
                style={{ width: '100%', height: 48, fontSize: 15, fontWeight: 700, borderRadius: 12, boxShadow: '0 8px 24px rgba(245,158,11,0.25)' }} 
                icon={<Calendar size={18} />}
                onClick={() => window.open(instructor.appointment_link, '_blank')}
              >
                Book Office Hour
              </Btn>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 350px) 1fr', gap: 24, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <Card style={{ padding: 24, background: 'linear-gradient(180deg, rgba(255,255,255,0.03) 0%, rgba(255,255,255,0.01) 100%)', border: '1px solid rgba(245,158,11,0.1)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 16, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, color: 'var(--text-secondary)', fontSize: 14 }}>
                  <BookOpen size={16} className="text-amber" /> Assigned Courses
                </div>
              ...
              {isAdmin && (
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 16, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, color: 'var(--text-secondary)', fontSize: 14 }}>
                      <Building2 size={16} className="text-amber" /> Departments
                    </div>
                    <div style={{ fontSize: 18, fontWeight: 800, color: '#fff' }}>{totalDepartments}</div>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, color: 'var(--text-secondary)', fontSize: 14 }}>
                      <School size={16} className="text-amber" /> Faculties
                    </div>
          ...
          <Card style={{ padding: 24, background: 'rgba(255,255,255,0.02)' }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10, color: '#fff' }}>
              <Clock size={18} className="text-amber" /> Contact Hours
            </h3>
            <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.7, whiteSpace: 'pre-wrap', background: 'rgba(0,0,0,0.2)', padding: '16px', borderRadius: 12, border: '1px solid var(--border)' }}>
              {instructor.office_hours || 'No standard contact hours specified for this term.'}
            </div>
          </Card>

          {/* Elevated Overrides for Engineer */}
          {instructor.capabilities && instructor.capabilities.length > 0 && (
            <Card style={{ padding: 24, background: 'linear-gradient(180deg, rgba(74, 142, 255, 0.05) 0%, rgba(74, 142, 255, 0.01) 100%)', border: '1px solid rgba(74, 142, 255, 0.2)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {instructor.capabilities.map((cap, i) => {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <Card style={{ padding: 28, background: 'rgba(255,255,255,0.02)' }}>
            <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10, color: '#fff' }}>
              <UserCircle2 size={20} className="text-amber" /> Biography
            </h3>
            <div style={{ fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.8 }}>
              {instructor.bio || `${instructor.name} is a dedicated engineering faculty member at the university, deeply involved with the practical and theoretical training of their students.`}
            </div>
          </Card>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
               <Card style={{ padding: 24, position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, position: 'relative', zIndex: 1 }}>
                    {instructor.faculties?.length > 0 ? instructor.faculties.map(f => (
               ...
               <Card style={{ padding: 24, position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, position: 'relative', zIndex: 1 }}>
                    {instructor.departments?.length > 0 ? instructor.departments.map(d => (
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, padding: '0 8px' }}>
              <h3 style={{ fontSize: 20, fontWeight: 700, margin: 0, display: 'flex', alignItems: 'center', gap: 10, color: '#fff' }}>
                <BookOpen size={22} className="text-amber" /> Assigned Modules
              </h3>
              <Badge color="amber" style={{ padding: '4px 12px', fontSize: 13 }}>{assigned_courses.length} Active</Badge>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {assigned_courses.length === 0 ? (
                <Card style={{ padding: '60px 40px', textAlign: 'center', background: 'rgba(255,255,255,0.01)', borderStyle: 'dashed' }}>
                  <BookOpen size={48} style={{ color: 'var(--text-muted)', opacity: 0.5, marginBottom: 16 }} />
                  <div style={{ fontSize: 16, fontWeight: 600, color: '#fff', marginBottom: 8 }}>No assigned modules</div>
                  <div style={{ color: 'var(--text-muted)', fontSize: 14 }}>This instructor does not currently lead any courses.</div>
                </Card>
              ) : shouldGroup ? (
                uniqueDepts.map((deptId) => {
                  ...
                  return (
                    <Card key={deptId} style={{
                      ...
                      border: isExpanded ? '1px solid rgba(245,158,11,0.3)' : '1px solid var(--border)',
                      ...
                      <div 
                        ...
                        style={{ 
                          ...
                          cursor: 'pointer', background: isExpanded ? 'rgba(245,158,11,0.05)' : 'transparent' 
                        }}
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                          <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(245,158,11,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--amber)' }}>
                            <Building2 size={20} />
                          </div>
                          <div>
                            <div style={{ fontWeight: 700, fontSize: 16, color: '#fff' }}>{deptName}</div>
                            <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>
                              <strong style={{ color: 'var(--amber)', fontWeight: 700 }}>{deptCourses.length}</strong> modules supervised
                            </div>
                          </div>
                        </div>
